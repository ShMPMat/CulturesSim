...
data class ResourceTemplateLink(val resourceName: String, val transformer: ResourceTransformer?, val amount: Int) {
    fun transform(resource: Resource) =
        (transformer?.transform(resource) ?: resource).copy(amount)
}

fun parseLink(tag: String, conversionParser: ConversionParser): ResourceTemplateLink {
    ...
    return ResourceTemplateLink(name, transformer, amount)
}
