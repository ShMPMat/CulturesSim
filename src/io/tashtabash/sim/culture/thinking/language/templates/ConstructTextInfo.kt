...
import io.tashtabash.sim.culture.group.centers.CultureCenter
import io.tashtabash.sim.culture.group.centers.util.ArbitraryResource
import io.tashtabash.sim.culture.thinking.meaning.*
...
fun getResourceInformationTextInfo(resource: Resource): List<TextInfo> {
    ...
    for (resourceDependency in resource.genome.dependencies)
        if (resourceDependency is ConsumeDependency)
            for (res in resourceDependency.lastConsumed(resource.baseName).keys) {
                infos.add(TextInfo(
